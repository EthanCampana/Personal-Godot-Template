...
class LoadedSceneData:
	...
	func newSceneData(load_into: Node, transition: String) -> LoadedSceneData:
...
@onready
var TransitionMgr: Transitions = preload("res://Scenes/TransitionManager.tscn").instantiate()
...
func check_scene_cache(scene_path: String) -> bool:
	_verify_scene_path(scene_path)
	if ResourceLoader.has_cached(scene_path):
		return true
	else:
		if _loaded_scenes_cache.has(scene_path):
			_loaded_scenes_cache.erase(scene_path)
		return false


func load_scene_into_cache(
	scene_to_load: String, load_into: Node = null, transition: String = "fadeIn"
) -> void:
	if _load_in_progress:
		push_warning("Scene Manager is loading something already")
		push_warning("Add Request to queue")
		_cache_queue.append(
			{"scene_path": scene_to_load, "load_into": load_into, "transition": transition}
		)
		return
	_cache_queue.append(
		{"scene_path": scene_to_load, "load_into": load_into, "transition": transition}
...
func swap_scene_cache(cache_scene_path: String, unload_scene: Node = null) -> void:
	var lsd: LoadedSceneData = _loaded_scenes_cache.get(cache_scene_path)
	if lsd == null:
		_content_not_exist_cache.emit(cache_scene_path)
		return
	var load_into = lsd.load_into
	var new_scene = ResourceLoader.load(cache_scene_path, "", ResourceLoader.CACHE_MODE_REUSE)
	load_into.add_child(new_scene.instantiate())
	TransitionMgr.transition(lsd.transition)
	await TransitionMgr.transition_finished
	TransitionMgr.transition_out()
	_unload_Node(unload_scene)
	get_tree().root.add_child(load_into)
	scene_added.emit()
	return


## NOTE: This function loads the new scene but will not save it to the ResourceLoader cache. [br]
## If you are looking to have your scene cached, use load_scene_into_cache and swap_scene_cache to get the desired result [br]
func load_scene_and_swap(
	scene_to_load: String,
	load_into: Node = null,
	outgoing_scene: Node = null,
	transition: String = "fadeIn"
) -> void:
	if _load_in_progress:
		push_warning("Scene Manager is loading something already")
		return
...
func _load_content_threaded() -> void:
	var loader = ResourceLoader.load(_scene_to_load, "PackedScene", ResourceLoader.CACHE_MODE_REUSE)
...
func _monitor_threaded_load() -> void:
	...
	match load_status:
		...
		ResourceLoader.THREAD_LOAD_LOADED:
			...
			_loaded_scenes_cache[_scene_to_load] = _load_scene_data.newSceneData(
				_load_into, _transition
			)
...
func _swap_scenes(unload_scene: Node = null) -> void:
	var new_scene = ResourceLoader.load(_scene_to_load, "", ResourceLoader.CACHE_MODE_IGNORE)
